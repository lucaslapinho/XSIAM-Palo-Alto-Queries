...
Identify policy-exception candidates involving developer tooling software after product identity, current inventory state, endpoint scope, and policy have been independently established.

## Source query

Primary input: [`cyber-hygiene/development-ides/README.md`](../../cyber-hygiene/development-ides/README.md). The query supplies reported inventory candidates only. It does not prove execution, use, current installation, or policy violation.
...
Join or compare a corroborated developer tooling product candidate with an organization-specific policy record using stable product ID plus asset/user/business scope, version and effective time. Emit only policy mismatches or expired exceptions.
...
Review asset role, approved engineering population and repository access. The same product can be expected, required, tolerated, or prohibited in different asset/user/business scopes.
