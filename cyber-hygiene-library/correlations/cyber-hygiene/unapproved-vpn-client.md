...
Identify policy-exception candidates involving VPN/tunneling/proxy software after product identity, current inventory state, endpoint scope, and policy have been independently established.

## Source query

Primary input: [`cyber-hygiene/vpn-tunneling-proxy/README.md`](../../cyber-hygiene/vpn-tunneling-proxy/README.md). The query supplies reported inventory candidates only. It does not prove execution, use, current installation, or policy violation.
...
Join or compare a corroborated VPN/tunneling/proxy product candidate with an organization-specific policy record using stable product ID plus asset/user/business scope, version and effective time. Emit only policy mismatches or expired exceptions.
...
Review approved destinations, split tunneling and network controls. The same product can be expected, required, tolerated, or prohibited in different asset/user/business scopes.
