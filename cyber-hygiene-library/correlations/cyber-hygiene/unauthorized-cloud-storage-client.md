# Unauthorized Cloud Storage Client
...
Identify policy-exception candidates involving cloud storage/synchronization software after product identity, current inventory state, endpoint scope, and policy have been independently established.

## Source query

Primary input: [`cyber-hygiene/cloud-storage/README.md`](../../cyber-hygiene/cloud-storage/README.md). The query supplies reported inventory candidates only. It does not prove execution, use, current installation, or policy violation.
...
Join or compare a corroborated cloud storage/synchronization product candidate with an organization-specific policy record using stable product ID plus asset/user/business scope, version and effective time. Emit only policy mismatches or expired exceptions.
...
Review managed account, shared folders and data classification. The same product can be expected, required, tolerated, or prohibited in different asset/user/business scopes.
