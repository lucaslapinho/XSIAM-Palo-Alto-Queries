...
Identify policy-exception candidates involving AI client software after product identity, current inventory state, endpoint scope, and policy have been independently established.

## Source query

Primary input: [`cyber-hygiene/ai-tools/README.md`](../../cyber-hygiene/ai-tools/README.md). The query supplies reported inventory candidates only. It does not prove execution, use, current installation, or policy violation.
...
Join or compare a corroborated AI client product candidate with an organization-specific policy record using stable product ID plus asset/user/business scope, version and effective time. Emit only policy mismatches or expired exceptions.
...
Review approved account tenancy, submitted data, retention and connectors. The same product can be expected, required, tolerated, or prohibited in different asset/user/business scopes.
