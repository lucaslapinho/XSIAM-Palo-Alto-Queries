# Successful RemoteInteractive Logon Records - Schema Binding Template
...
[Open query](n075-successful-remoteinteractive-logon-records-schema-binding-template.template.xql) · [Category index](README.md) · [Library](../README.md)
...
|---|---|---|
| positive | A Security 4624 from the Security-Auditing provider has numeric LogonType 10. | Returned. |
| negative | LogonType 3, a network connection on 3389 or failed event 4625 occurs. | Excluded. |
| null | Type-10 event has a unique ID but null client IP. | Returned with null source and limited attribution. |
| edge | The same source event is delivered twice. | One retained event; distinct event UIDs are not collapsed by timestamp. |
