...
| filter process_name in ("anydesk.exe", "meshagent.exe", "mstsc.exe", "plink.exe", "pscp.exe", "psexec.exe", "psexec64.exe", "psftp.exe", "putty.exe", "rustdesk.exe", "srfeature.exe", "srmanager.exe", "teamviewer.exe", "teamviewer_desktop.exe", "teamviewer_service.exe", "tvnserver.exe", "tvnviewer.exe")
...
| filter tool_family = "Remote Access"
| comp count() as remote_access_rows, values(detected_product) as remote_products, values(agent_hostname) as hostnames by agent_id
| join type = inner conflict_strategy = left (
    dataset = xdr_data
    | filter event_type = PROCESS and event_sub_type = PROCESS_START
    | filter action_process_image_name in ("powershell.exe", "pwsh.exe", "wscript.exe", "cscript.exe", "python.exe", "pythonw.exe")
    | comp count() as script_rows by agent_id
) as scripting scripting.agent_id = agent_id
| fields agent_id, hostnames, remote_products, remote_access_rows, script_rows
| sort desc remote_access_rows
| limit 500
