...
| filter actor_process_image_name != null and actor_process_image_name != ""
| comp count() as process_start_rows,
       count_distinct(agent_id) as distinct_endpoint_count,
       values(agent_hostname) as hostnames,
       min(_time) as first_observed,
       max(_time) as last_observed
    by tool_family, detected_product, actor_process_image_name
| filter distinct_endpoint_count <= 2
| sort asc distinct_endpoint_count
| limit 500
