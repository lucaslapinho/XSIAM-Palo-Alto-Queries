...
| comp count_distinct(detected_product) as distinct_product_count,
       values(detected_product) as products,
       values(endpoint_name) as hostnames
    by endpoint_id, tool_family
| filter distinct_product_count >= 2
| sort desc distinct_product_count
| limit 500
